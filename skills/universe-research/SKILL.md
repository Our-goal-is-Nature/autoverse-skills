...
- Agent 做主题检索时，必须同时写入 `--year-from` 与 `--year-to`。默认起始年等于当前公历年减 4，默认结束年等于当前公历年，起止两年都计入。以 2026 年为例，Agent 写入 `--year-from 2022` 与 `--year-to 2026`。参数名称以 [../autoverse-cli/references/commands.md](../autoverse-cli/references/commands.md) 为准。Agent 不得使用已经删除的 `--year`。检索结果噪声大时，Agent 只收窄术语，或改用该学科 reference 已经列出的别名。Agent 在未说明原因时，不得删掉 `--year-from`，也不得删掉 `--year-to`。
