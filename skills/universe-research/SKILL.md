...
- 主题检索以近五年为主。Agent 按该学科 reference 中的检索角度拆开总问题，对各角度分别检索时，均须加 `--year-from` 与 `--year-to`，闭区间为当前年份往前五年（含当年）。参数名以 [../autoverse-cli/references/commands.md](../autoverse-cli/references/commands.md) 为准。Agent 不得使用已删除的 `--year`。宽检索式噪声大时，Agent 收窄术语或改用该学科 reference 列出的别名，不得拿掉 `--year-from` 与 `--year-to`。
- 题目的奠基研究仍纳入，但只走定点：用完整题名找回某一已知工作，不得加 `--year-from` 或 `--year-to`。Agent 不得把奠基扩成第二轮无窗主题检索。奠基只补本题仍在使用的对象定义或框架，不得用更早的高被引篇目填满主题各节。
- 近五年某一角度未能返回可比较集合时，Agent 在文献来源中写明该角度的实际时限与篇数，不得改为无窗检索补篇，也不得把未经近五年检索的内容写成该角度的近期进展。
...
2. **文献来源**：检索途径（Autoverse 文献检索；若使用了官方页面补充，一并写明来源）、主题检索的年份闭区间、近五年纳入篇数、奠基纳入篇数、别名如何纳入、判断依据摘要。某一角度在近五年闭区间内未能形成可比较集合时，写明该角度的实际覆盖。不写命令名，不写核验题录过程。文献来源中的学科表述以对应 reference 为准。
